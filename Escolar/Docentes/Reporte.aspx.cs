    ...
    public partial class Reporte : System.Web.UI.Page
    {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    {
                        lblMessage.Text = "No tiene permisos para actualizar este reporte.";
                    }
                    LoadReportes();
                }
            }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                {
                    lblMessage.Text = "No tiene permisos para eliminar este reporte.";
                }
                LoadReportes();
            }
        }
        ...
        private void LoadReportes()
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand("SELECT idReporte, tipo, detalle, fecha, CONCAT(e.nombre, ' ', e.paterno, ' ', e.materno) AS nombreCompleto FROM reporte r JOIN estudiante e ON r.idEstudiante = e.matricula WHERE r.creadoPor = @creadoPor", con);
                cmd.Parameters.AddWithValue("@creadoPor", User.Identity.Name);
        ...
        private void LoadEstudiantes()
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand("SELECT e.matricula, e.nombre + ' ' + e.paterno + ' ' + e.materno AS nombreCompleto FROM estudiante e JOIN grupoEstudiante ge ON e.matricula = ge.idEstudiante JOIN grupoMateria gm ON ge.idGrupo = gm.idGrupo JOIN materia m ON gm.idMateria = m.idMateria WHERE m.idDocente = (SELECT idDocente FROM docente WHERE idUsuario = (SELECT Id FROM AspNetUsers WHERE UserName = @userName))", con);
